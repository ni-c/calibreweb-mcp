...
export function buildToolFilter(config: {
  ...
  if (allow === undefined) {
    ...
    for (const entry of allow) {
      ...
      if (matches.length === 0) {
        throw new ToolFilterError(
          `CALIBRE_WEB_ALLOW_TOOLS: no tool matches "${entry}". ` +
  ...
  for (const entry of deny ?? []) {
    ...
    if (matches.length === 0) {
      throw new ToolFilterError(
        `CALIBRE_WEB_DENY_TOOLS: no tool matches "${entry}". Valid tools: ${catalogueList()}.`
      );
    }
  ...
  if (selected.size === 0) {
    throw new ToolFilterError(
      'CALIBRE_WEB_ALLOW_TOOLS/CALIBRE_WEB_DENY_TOOLS leave no tools registered — the ' +
